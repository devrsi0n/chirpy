...
export function GQLClientProvider({ children }: GqlClientProviderProps): JSX.Element {
  const { data: session } = useSession();
  const [client, setClient] = React.useState(() => createGqlClient(session?.hasuraToken));
  React.useEffect(() => {
