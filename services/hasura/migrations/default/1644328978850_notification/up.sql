...
CREATE FUNCTION public."set_current_timestamp_deletedAt"() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  _new record;
...
COMMENT ON COLUMN public."NotificationMessage"."contextId" IS 'Triggered entity''s id, e.g. CommentId or LikeId';
...
COMMENT ON TABLE public."NotificationSubscription" IS 'Registered subscriptions for notification';
...
ALTER TABLE ONLY public."NotificationMessage"
    ADD CONSTRAINT "NotificationMessage_contextId_type_recipientId_triggeredById_ke" UNIQUE ("contextId", type, "recipientId", "triggeredById");
ALTER TABLE ONLY public."NotificationMessage"
    ADD CONSTRAINT "NotificationMessage_pkey" PRIMARY KEY (id);
ALTER TABLE ONLY public."NotificationSubscription"
    ADD CONSTRAINT "NotificationSubscription_pkey" PRIMARY KEY (id);
ALTER TABLE ONLY public."NotificationSubscription"
    ADD CONSTRAINT "NotificationSubscription_subscription_key" UNIQUE (subscription);
ALTER TABLE ONLY public."NotificationType"
    ADD CONSTRAINT "NotificationType_pkey" PRIMARY KEY (value);
...
CREATE TRIGGER "set_public_NotificationMessage_deletedAt" BEFORE UPDATE ON public."NotificationMessage" FOR EACH ROW EXECUTE FUNCTION public."set_current_timestamp_deletedAt"();
COMMENT ON TRIGGER "set_public_NotificationMessage_deletedAt" ON public."NotificationMessage" IS 'trigger to set value of column "deletedAt" to current timestamp on row update';
...
ALTER TABLE ONLY public."NotificationMessage"
    ADD CONSTRAINT "NotificationMessage_recipient_fkey" FOREIGN KEY ("recipientId") REFERENCES public."User"(id) ON UPDATE CASCADE ON DELETE RESTRICT;
ALTER TABLE ONLY public."NotificationMessage"
    ADD CONSTRAINT "NotificationMessage_triggerBy_fkey" FOREIGN KEY ("triggeredById") REFERENCES public."User"(id) ON UPDATE CASCADE ON DELETE RESTRICT;
ALTER TABLE ONLY public."NotificationMessage"
    ADD CONSTRAINT "NotificationMessage_type_fkey" FOREIGN KEY (type) REFERENCES public."NotificationType"(value) ON UPDATE CASCADE ON DELETE RESTRICT;
ALTER TABLE ONLY public."NotificationSubscription"
    ADD CONSTRAINT "NotificationSubscription_userId_fkey" FOREIGN KEY ("userId") REFERENCES public."User"(id) ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE ONLY public."Page"
